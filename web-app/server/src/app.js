...
app.post('/registerManufacturer', async (req, res) => {
  ...
  let response = await network.registerManufacturer(voterId, req.body.registrarId, req.body.firstName, req.body.lastName);
  console.log('response from registerManufacturer: ');
  console.log(response);
  if (response.error) {
    res.send(response.error);
  } else {
